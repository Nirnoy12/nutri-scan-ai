...
class Scan(db.Model):
    ...
    quick_verdict = db.Column(
        db.String(1000), nullable=False
    )  # Increased size for AI verdicts
    ocr_text = db.Column(db.Text, nullable=True)  # Will store combined OCR or food list
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)


# ==================================
# --- CRITICAL FIX: FORCE DB CREATION HERE ---
# ==================================
# This ensures the tables are created when gunicorn loads the app module,
# and AFTER all models (User, Scan) have been defined.
logging.info("--- RENDER DEPLOY FIX: EXECUTING db.create_all() ---")
...
def extract_text(image_path):
    ...
    try:
        ...
        prompt_text = (
            "Extract all text from this image. Respond with only the transcribed text."
        )
...
def recognize_food(image_path):
    ...
    try:
        ...
        if ":" in response_text:
            response_text = (
                response_text.split(":")[-1].strip().replace('"', "").replace(".", "")
            )
...
def get_history():
    ...
    try:
        ...
        logging.debug(
            f"History fetched for user {current_user.id}: {len(history_list)} scans"
        )
...
def analyze_image():
    ...
    if not (
        ...
        return jsonify(
            {
                "error": f"Invalid file type. Please upload a (png, jpg, jpeg, webp) file. Got: {file.filename}"
            }
        ), 400
    ...
    if file_size > 10 * 1024 * 1024:  # 10MB
        ...
        return jsonify(
            {"error": f"File too large (Max 10MB). Got: {file_size} bytes"}
        ), 400

    if scan_type not in ["label", "food"]:
        logging.error(f"Invalid scan_type: {scan_type}")
        return jsonify(
            {"error": f"Invalid scan_type. Must be 'label' or 'food'. Got: {scan_type}"}
        ), 400
    ...
    try:
        if scan_type == "label":
            ...
            verdict, err = get_ai_nutrition_analysis(
                ocr_text_to_save, verdict_sys_prompt, verdict_user_prompt
            )
            if err:
            ...
            report, err = get_ai_nutrition_analysis(
                ocr_text_to_save, report_sys_prompt, report_user_prompt
            )
            if err:
                logging.error(f"AI Report failed: {err}")
                detailed_report = [
                    {
                        "nutrient": "Long-Term Impact",
                        "impact": f"Failed to generate report: {err}",
                    }
                ]
            else:
            ...
            ocr_text_to_save = (
                best_pick_text  # This is the clean string, e.g., "Hamburger"
            )

            # 2. Get AI Quick Verdict (Unchanged)
            verdict_sys_prompt = (
                "You are a professional nutritionist. A food item has been identified."
            )
